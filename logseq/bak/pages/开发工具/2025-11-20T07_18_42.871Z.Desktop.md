...
- FTP
  collapsed:: true
	- 主动模式和被动模式
		- 首先明确的一点是，客户端和服务器端交换分为两块，一块是命令交互，一块是数据交互，命令交互采用的是 21 端口，也就是说服务器段必须开通 21 端口供客户端进行调用。数据交互的不同进而区分了主动和被动模式。
	...
	- 如何选择主动模式和被动模式
		- 如果把FTP服务器部署在防火墙或者NAT服务器的背后，FTP服务是没有独立的公网IP地址，服务器想要主动连接客户端，难度太大，所以这种情况下建议使用被动模式。
		  这里就有一个实际的案例，在项目开发中，我们将网络区域分为DMZ和BIZ两部分，如果我的应用作为客户端连接外网三方的ftp服务器端。
			- 第一种方案是我们在DMZ区设置一个nginx，在nginx配置中监听ftp的请求和响应端口，但是采用这种方式是很困难的，因为这种情况ftp一般都是被动模式，但是我们在nginx中是无法配置监听ftp响应的端口的，所以这种方式是无法实现数据传输的。
