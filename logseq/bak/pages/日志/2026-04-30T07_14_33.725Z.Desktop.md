...
- Log4j2
	...
	- Log4j2 Appenders
		...
		- RoutingAppender
		  collapsed:: true
			- 根据 ThreadContext（MDC）里的 tradeName 值，动态选择 RollingFile Appender
	- Filters
	  collapsed:: true
	...
	- Strategy策略
		...
		- 日志归档的保留策略：Delete on Rollove（滚动删除）
			- 配置参数
				...
				- maxDepth：由于备份文件保存在`${LOG_HOME}/archive/$${date:yyyy-MM}/application-2026-02-01-1.log.gz`，所以目录深度设置为2，就可以实现覆盖删除
				  id:: 69887ce9-3c2f-4462-b5b2-0d112442bd2e
					- 第0级：`${LOG_HOME}/archive`
					- 第1级：`${LOG_HOME}/archive/$${date:yyyy-MM}`
					- 第2级：`${LOG_HOME}/archive/2026-02/application-xxx.log.gz`
					- 第3级：`${LOG_HOME}/archive/2023-10/01/app.log`
				- IfFileName：匹配文件名称
