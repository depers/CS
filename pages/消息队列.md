- RabbitMQ
			...
			- RabbitMQ Client的API中
				- `Channel.basicQos(int prefetchSize, int prefetchCount, boolean global)`
				  collapsed:: true
					...
					- `global`：指示是否将设置应用于整个连接，而不仅仅是当前通道。简单点说就是：**是否将上面设置应用于channel，就是上面两个限制是 channel 级别的还是 consumer 级别。**RabbitMQ 采用 global=false 表示 QoS 设置应适用于每个消费者（对于通道上的新消费者；现有消费者不受影响），而 global=true 表示 QoS 设置应适用于每个通道。
			- Spring AMQP的API中
	...
	- 死信队列
		...
		- DLX，Dead-Letter-Exchange，死信交换器
			...
			- 值得注意的是：只有在消息有效时间内没有消费者消费这条消息，消息才会传递到死信队列。
		- 死信队列
			- 是指绑定死信交换机队列被称为死信队列。
	...
	- 延时队列
		...
		- RabbitMQ没有直接支持延时队列，但是可以通过DLX死信队列交换器和TTL属性模拟出延时队列
	...
	- Spring Boot中使用RabbitMQ
		...
		- 消息监听的两种方案
		  collapsed:: true
			- 单独声明`SimpleMessageListenerContainer`Bean，需要实现`MessageListenerAdapter`的子类，在子类中可以实现它的`getListenerMethodName`进而完成监听不同队列走不同逻辑的功能。
			  collapsed:: true
				- 这个方案有个问题，当我在配置文件中声明`listener`的属性的时候，Spring AMQP就会帮我们声明`SimpleMessageListenerContainer`，如果我自己又重新声明了
			- 直接在配置文件中对listener进行配置
		- 监听消息的关键配置
		  collapsed:: true
			- direct容器
			  collapsed:: true
				- `consumersPerQueue`：为每个配置的队列创建消费者的数量。
			- simple容器
			  collapsed:: true
				- `concurrentConsumers(concurrency)`：最初为每个 listener 启动的并发消费者的数量。
				- `maxConcurrentConsumers(max-concurrency)`：如果需要，按需启动的最大并发消费者数量。必须大于或等于 `concurrentConsumers`。
			- `prefetchCount(prefetch)`：可设置 consumer 每次接收到的信息数量，系统默认值为 250，这表示当 consumer 队列接收到 250 请求其状态皆为 unacked 时，broker server 将暂停向 consumer 发送消息，待消息处理后再继续。
		- 设置监听器类型
		  collapsed:: true
			- simple
			  collapsed:: true
				...
				- 线程、连接、队列、消费者的关系：一个**线程**固定绑定一个**channel**（连接），一个连接会收到多个**队列**的消息并将消息分发到对应的**consumer**中。
				- SimpleMessageListenerContainer 是经典的容器，使用 channel 共享，一旦某个 channel 关闭或重启，意味着每个队列 queue 中使用当前 channel 的 consumer 都会受到影响。
			- direct
			  collapsed:: true
				- 在 RabbitMQ 2.0 后，系统引入了 DirectMessageListenerContainer ，它允许每个 consumer 都有各自的对应的 channel 的，channel 只管理负责管理当前 consumer 的通道。这样令 consumer 运用更灵活，同时线程并没有跟 channel 绑定，而是由独立的线程池进行管理，这是更好地解决了 SimpleMessageListenerContainer 中上下文切换所带来的资源消耗问题。
