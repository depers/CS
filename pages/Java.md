				  ...
				  collapsed:: true
					...
					- 处理异步结算的结果
						...
						- `thenAccept()`：不需要从回调函数中获取返回结果，可以使用 `thenAccept()`。但是可以获取上一次计算的结果。示例代码如下：
						  ...
						  ```
						- `thenRun()`：不需要从回调函数中获取返回结果，可以使用 `thenAccept()` 或者 `thenRun()`。这两个方法的区别在于 `thenRun()` 不能访问异步计算的结果。示例代码如下：
						  ```java
						  CompletableFuture.completedFuture("hello!")
						          .thenApply(s -> s + "world!").thenApply(s -> s + "nice!").thenRun(() -> System.out.println("hello!"));//hello!
						  ```
						- `whenComplete()`：不需要从回调函数中获取结果，入参`BiConsumer`可以接收两个参数。
					- 异常处理
					...
					- 组合 CompletableFuture
						...
						- 如果想要实现 task1 和 task2 中的任意一个任务执行完后就执行 task3 的话，可以使用 `acceptEither()`和`acceptEitherAsync()`。这两个函数不同的是`acceptEither`会在先完成任务的那个线程继续执行，而`acceptEitherAsync`则会异步新起一个线程去做
