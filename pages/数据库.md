...
- 问题
  collapsed:: true
	- MySQL中text类型的存储和检索
	- MySQL中decimal类型的使用
	- [如何查看 MySQL 数据库容量大小，表容量大小，索引容量大小？](https://www.xtplayer.cn/mysql/how-to-get-the-sizes-of-the-tables-of-a-mysql-database/#%E6%9F%A5%E7%9C%8B-MySQL%E3%80%8C%E6%8C%87%E5%AE%9A%E5%BA%93%E3%80%8D%E4%B8%AD%E3%80%8C%E6%89%80%E6%9C%89%E8%A1%A8%E3%80%8D%E7%9A%84%E5%AE%B9%E9%87%8F%E5%A4%A7%E5%B0%8F)
	- 为什么数据库字段要设置为`not null`
	  collapsed:: true
		- 字段含义不一样，数据库列字段默认值就是null，`NULL`和**`NOT NULL`使用的空值**代表的含义是不一样，NULL可以认为这一列的值是未知的，空值则可以认为我们知道这个值，只不过他是空的而已。
		- 聚合函数`count(字段)`，不会统计`null`值的数据。
		- 不能使用`=`来匹配`null`值，只能使用`is null`。
		- 与其他值的计算，比如`int类型+1`，字符类型的`concat()操作`后值仍为null。
		- 存在索引失效的问题。
		- 参考文章：[为什么数据库字段要使用NOT NULL？](https://cloud.tencent.com/developer/article/1812479)
	- max_binlog_cache_size
		- 问题：Multi-statement transaction required more than 'max_binlog_cache_size' bytes of storage; increase this mysqld variable and try again
		- 问题原因：max_binlog_cache_size参数指定了单个事务最大允许使用的Binlog，当超出这个值时，会出现当前报错。
		- 问题解决
			- 查看数据库配置：`show variables like 'max_binlog_cache_size';`，这个参数默认配置是2的64次方个字节，是很大的，最小值是4096，也就是4kb
			- 方法一：就是`set global max_binlog_cache_size xxxx`，这个值只能设置`global`维度。
			- 方法二：就是将`update`语句进行拆分，使用`limit`来做。
- 面经
  collapsed:: true
	- [这些年背过的面试题 —— MySQL篇](https://mp.weixin.qq.com/template/article/1708312042/index.html)
- SQL优化
  collapsed:: true
	- 分页查询使用`maxId`优化`limit`
	- 批量插入使用原生JDBC的`executeBatch()`和`rewriteBatchedStatements=true`
		- 参考文章：[调优 mybatis saveBatch 25倍性能](https://blog.csdn.net/yessimida/article/details/129858732)
		...
		- 事务的隔离级别与MVCC
			...
			- 数据并发导致的问题
				- 3类数据读问题
					...
					- 幻象读
						- A事务第一次读取了数据
						- B事务已经提交了事务，新增了数据（新增或是删除）
						  id:: 65fac63d-bfab-4edb-a618-0717478264e9
						- A事务第二次读取数据，发现与第一次读取的不一样
					- 不可重复读和幻象读的区别
						- **不可重复读**是同一条记录的内容被修改了，重点在于**UPDATE**，关注的是**同一行数据**在事务中的多次读取结果不一致。
						- **幻读**是查询某一个范围的数据行变多了或者少了，重点在于**INSERT**或**DELETE**，关注的是**同一范围数据**在事务中的多次查询结果不一致。
						- 三采取的解决策略不同
			...
			- MVCC
				...
				- ReadView
					...
					- ReadView
						- 作用：隔离性的控制内部实现是通过**判断版本链中的哪个版本是当前事务可见的**。当前查询事务对于生成ReadView**之前未提交的事务**或者**之后才开启的事务**所做的更改是看不到的。
						- ReadView生成的主要内容
	  ...
	  collapsed:: true
				...
				- 写操作
					...
					- UPDATE
						...
						- 若修改了该记录行的键值
							...
							- 2.进行INSERT操作。
					- INSERT
						- 一般情况下，新插入一条记录受隐式锁保护，不需要内存为其生成对应的锁结构。
		- 2.多粒度锁
			- 表级锁
				- 共享锁和独占锁
					- 共享锁（S锁）
					- 独占锁（X锁）
				- 意向锁（Intention lock）
					- 作用：IS锁和IX锁是表级锁，是为了在添加**表级别**的S锁和X锁时可以快速的判断出表中的记录是否上锁，以避免使用遍历的方式来查看表中有没有上锁的记录。也就是说IS锁和IX锁是兼容的，IX锁和IX锁也是兼容的。
		...
		- 3.MySQL中的行锁和表锁
			...
			- InnoDB引擎中的行级锁
				- 行级锁主要有以下四种
				- 记录锁-Record Lock
					- 作用：仅仅是将一条记录锁上，所以记录锁是**查询条件是添加了唯一索引或是主键索引的列**，比如：`select * from user where id = 1 for update;`
