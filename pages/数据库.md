- 数据库基础
	- 事务
		...
		- 具体实现
			- 数据库采用**重执行日志**来保证**原子性**、**一致性**和**持久性**。
			- 数据库采用数据库**锁机制**保证事务的**隔离性**。Oracle数据库还采用了数据版本的机制去保证隔离性。
	- 数据并发导致的问题
	  collapsed:: true
		- 3类数据读问题
			- 脏读（dirty read）
			...
			- 不可重复读和幻象读的区别
				- 一幻象读一般发生在计算统计数据的事务中
				- 二幻象读是另一个事务进行了**新增数据**的操作，而不可重复读是进行了**数据更新和删除**的操作
				- 三采取的解决策略不同
	...
	- 第二十一章 undo日志
		...
		- 回滚段
			...
			- 从回滚段中获取undo页面链表
				...
				- 当一个事务提交时，它所占用的`undo slot`有两种命运（为页面重用做准备）：
					- 如果该`undo slot`指向的`Undo页面`链表符合被重用的条件
						...
						- 根据`undo页面`所在链表的类型的不同，`undo页面`对应的`undo slot`会分别放置到`insert undo cached`链表和`update undo cached`链表中。
			...
			- 问题：回滚段和上面提到的Undo Log Segment的区别
			  collapsed:: true
				- 回滚段是一个页面，Undo Log Segment是一个逻辑结构，这个段与表空间的段时一样的，页面都是从段中申请的。
		- 多个回滚段
		  collapsed:: true
		...
		- 回滚段的分类
			- 对普通表的记录做了改动需要分配undo页面链表涉及的回滚段
			- 对临时表的记录做了改动需要分配undo页面链表涉及的回滚段
